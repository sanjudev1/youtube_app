...
import { YOUTUBE_SUGGESTION_API } from '../constants/apis'
...
function Header() {
  ...
  return (
    <div className='grid grid-flow-col p-2 shadow-md fixed top-0 left-0 w-full bg-slate-50 z-10'>
      ...
      <div className='flex' >
        <input type="text" placeholder='Search' className='flex border border-gray-400 rounded-l-full w-1/2 h-10 justify-center align-middle pl-7' value={searchQuery} onChange={(e)=>setSearchQuery(e.target.value)} onFocus={()=>setShowSuggestions(true)} onBlur={()=>setShowSuggestionsdelay()}/>
      ...
      {(suggestions.length!==0 && showsuggestions) && <div className=' boder bg-gray-100 rounded-lg shadow-lg w-[41rem] ml-2 py-3 fixed top-55 left-55 z-100' >
        <ul>{suggestions.map(s=><li className='hover:bg-gray-300 font-semibold'key={s} ><a href={"search?v="+s}><button className='m-2'><img className="h-4 mt-3 ml-3" src="https://upload.wikimedia.org/wikipedia/commons/thumb/0/0b/Search_Icon.svg/1024px-Search_Icon.svg.png" alt="search"/></button>{s}</a></li>)}</ul></div>}
      </div>

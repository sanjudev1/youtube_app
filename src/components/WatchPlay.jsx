...
function WatchPlay() {
  ...
  return (
    <div className="w-full scroll-top">
      <div className="flex">
        <div className={menuOption === true ? 'ml-44' : 'm-5'}>
          <iframe
            width="1024"
            height="524"
        ...
        <div className="w-full">
          <div className="text-black bg-white w-[25rem] h-[32rem] rounded-sm m-5 overflow-y-auto  flex flex-col-reverse">
            {chatMessages.map((each, index) => (
              <div key={index} className="flex ml-3">
                <img
                  ...
                  alt="profile-icon"
                />
                <div className="font-bold ml-3 mt-1">
                  <h1>{each.name}</h1>
                  <h1 className="font-semibold">{each.text}</h1>
                </div>
              </div>
            ))}
          </div>
          <div>
            <form
              onSubmit={(e) => {
                e.preventDefault(),
                  dispatch(
                    Addmessages({ name: 'Anonymous User', text: postmsg })
                  ),
                  console.log(postmsg),
                  setpostmg('');
              }}
            >
              <input
                type="text"
                className="p-1 w-[20rem] ml-5"
                placeholder="chat with live..."
                value={postmsg}
                onChange={(e) => setpostmg(e.target.value)}
              />{' '}
              <button className="bg-white w-[98px] h-[34px] rounded-md text-lime-900">
                Send
              </button>
            </form>
          </div>
